...
(function(){
  ...
  // Chips da faixa do hero: cada um leva ao card correspondente do
  // carrossel "Soluções especializadas" (via data-slide); o chip "Outros"
  // não tem data-slide, então mantém o link genérico para a seção.
  var heroChips = Array.prototype.slice.call(document.querySelectorAll('.hero-chip[data-slide]'));
  heroChips.forEach(function(chip){
    chip.addEventListener('click', function(){
      var i = parseInt(chip.getAttribute('data-slide'), 10);
...
(function(){
  ...
  var items = Array.prototype.slice.call(document.querySelectorAll('.reveal, .reveal-rule'));
  ...
  // Alterna is-visible nos dois sentidos (sem unobserve): o elemento
  // reaparece ao entrar na tela e volta a se esconder ao sair, para poder
  // ser revelado de novo da próxima vez.
...
(function(){
  // Cabeçalho compacto ao rolar e botão "voltar ao topo". Cada peça é
  // opcional: se um elemento não existir no HTML, essa parte simplesmente
  // não faz nada.
